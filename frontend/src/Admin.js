...
useEffect(()=>{
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEditing) {
            ...
            try {
                const response = await fetch(`http://localhost:5000/api/products/${currentId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const markAsRead = async (id) => {
        try {
            const response = await fetch(`http://localhost:5000/api/contacts/${id}/read`, { method: 'PUT' });
    ...
    const deleteMessage = async (id) => {
        if (window.confirm("Are you sure you want to delete this?")) {
            try {
                const response = await fetch(`http://localhost:5000/api/contacts/${id}`, { method: 'DELETE' });
